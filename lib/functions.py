...
def demultiplex(inQza,metadata,outdir):
    ...
    # fqMuBiSe
        ...
        call('qiime cutadapt demux-single --i-seqs '+inQza+' --m-barcodes-file '+metadata+' --m-barcodes-column barcode-sequence --o-per-sample-sequences '+outdir+'/'+re.sub(r'\.qza$','',os.path.basename(inQza))+'-demux.qza --o-untrimmed-sequences '+outdir+'/'+re.sub(r'\.qza$','',os.path.basename(inQza))+'-untrimmed.qza','Cutadapt and demultiplexing '+inQza+' done successfully.','Cutadapt and demultiplexing '+inQza+' failed.')
    ...
    # fqMuBiPe
        ...
        if 'barcode-sequence2' in df.columns:
            call('qiime cutadapt demux-paired --i-seqs '+inQza+' --m-forward-barcodes-file '+metadata+' --m-forward-barcodes-column barcode-sequence --m-reverse-barcodes-file '+metadata+' --m-reverse-barcodes-column barcode-sequence2 --o-per-sample-sequences '+outdir+'/'+re.sub(r'\.qza$','',os.path.basename(inQza))+'-demux.qza --o-untrimmed-sequences '+outdir+'/'+re.sub(r'\.qza$','',os.path.basename(inQza))+'-untrimmed.qza','Cutadapt and demultiplexing '+inQza+' done successfully.','Cutadapt and demultiplexing '+inQza+' failed.')
        else:
            call('qiime cutadapt demux-paired --i-seqs '+inQza+' --m-forward-barcodes-file '+metadata+' --m-forward-barcodes-column barcode-sequence --o-per-sample-sequences '+outdir+'/'+re.sub(r'\.qza$','',os.path.basename(inQza))+'-demux.qza --o-untrimmed-sequences '+outdir+'/'+re.sub(r'\.qza$','',os.path.basename(inQza))+'-untrimmed.qza','Cutadapt and demultiplexing '+inQza+' done successfully.','Cutadapt and demultiplexing '+inQza+' failed.')
...
def mergeMetadata(indir,out):
    metadataFile=[indir+'/'+i+'/metadata.tsv' for i in os.listdir(indir)]
